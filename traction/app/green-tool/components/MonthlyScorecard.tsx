...
export default function MonthlyScorecard({ data, chapterData, title }: { data: any[], chapterData?: any, title?: string }) {
  ...
  const displayTitle = title || `Monthly Scorecard (${chapterData?.monthYear})`;

  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6 h-full">
      <div className="mb-6">
        <h2 className="text-[17px] font-extrabold text-gray-900 tracking-tight">{displayTitle}</h2>
        <p className="text-[12px] text-gray-500 font-medium mt-0.5">Chapter totals &middot; monthly targets shown</p>
      </div>
