...
export default function AdminPage() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await fetch('/nearby/api/admin/users');
  ...
  const deleteUser = async (userId: string) => {
    ...
    setUpdatingId(userId);
    try {
      const res = await fetch(`/nearby/api/admin/users?userId=${userId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    setInviteMessage('');
    try {
      const res = await fetch('/nearby/api/admin/users/invite', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const res = await fetch('/nearby/api/admin/users/bulk-invite', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
