...
import { useLocationStore, DEFAULT_COORDS, DEFAULT_CITY } from '@/nearby-stores/nearby-location-store';
import { MapView } from '@/components/nearby-map/map-view';
import { MapSkeleton } from '@/components/nearby-ui/skeleton';
...
function DiscoverContent() {
  ...
  useEffect(() => {
    if (!authLoading && !user) router.push('/nearby/login');
  ...
  async function fetchMembers() {
    ...
    try {
      ...
      const res = await fetch(`/nearby/api/members?lat=${lat}&lng=${lng}&radius=50${categoryParam}`);
      const data = await res.json();
