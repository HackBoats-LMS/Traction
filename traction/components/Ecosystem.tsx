      ...
      <div className="max-w-7xl mx-auto flex flex-col justify-center items-center h-full">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6 max-w-5xl mx-auto w-full">
          {features.map((feature, idx) => (
            <div key={idx} className="bg-[#FBFBFB] rounded-xl p-4 md:p-6 lg:p-8 border border-black/10 shadow-sm flex flex-col items-center text-center hover:shadow-md transition-shadow">
              <div className="flex flex-row items-center justify-center gap-1.5 md:gap-2 mb-4 md:mb-6">
                <div className="scale-75 md:scale-90 lg:scale-100 shrink-0 text-green-600">
                  {feature.icon}
                </div>
                <span className="font-bold text-green-600 text-[10px] md:text-xs lg:text-sm whitespace-nowrap">{feature.label}</span>
              </div>
              <h4 className="font-bold text-[10px] md:text-xs lg:text-sm mb-1.5 md:mb-2 text-[#1D1D1F] whitespace-nowrap">{feature.title}</h4>
              <p className="text-[#555555] text-[8px] md:text-[10px] lg:text-xs whitespace-nowrap">{feature.desc}</p>
            </div>
          ))}
        </div>
      </div>
