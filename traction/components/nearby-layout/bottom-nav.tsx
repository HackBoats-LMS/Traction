...
const navItems = [
  { href: '/nearby/dashboard', label: 'Home', icon: LayoutDashboard },
  { href: '/nearby/discover', label: 'Map', icon: MapIcon },
  { href: '/nearby/discover?view=list', label: 'List', icon: List },
    ...
    <div className="fixed bottom-6 left-0 right-0 z-50 px-4 sm:px-6 pointer-events-none pb-safe">
      <div className="max-w-md mx-auto pointer-events-auto">
        <div className="bg-white/95 backdrop-blur-xl rounded-[32px] px-2 py-2 flex items-center justify-between shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-200/60">
          {navItems.map((item) => {
            let isActive = false;
            if (item.href === '/nearby/discover') {
              isActive = pathname === '/nearby/discover' && view !== 'list';
            } else if (item.href === '/nearby/discover?view=list') {
              isActive = pathname === '/nearby/discover' && view === 'list';
            } else {
